...
@CommandParameters(name = "saconfig", description = "Access and change admin information", usage = "/saconfig <list | <add | delete> <username> <rank level>>", rank = PM_Rank.Rank.OP)
public class Command_saconfig
{
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args)
    {
        ...
        if(args.length == 2)
        {
            ...
            {
                if(PM_Rank.isRankOrHigher(player, PM_Rank.getRank(sender)) && sender instanceof Player)
                {
                    sender.sendMessage("You may only remove someone from admin if they are a lower rank than you.");
                    return true;
                }
                PM_Rank.setRank(player, PM_Rank.Rank.OP, sender);
                PM_Utils.adminAction(sender.getName(), "Removing " + player.getName() + " from the admin list.", true);
                return true;
            }
            if(args[0].equalsIgnoreCase("add"))
            {
                if(PM_Rank.getRank(player).level == -1)
                {
                    try
                    {
                        Connection c = PM_SqlHandler.getConnection();
                ...
                PM_Rank.setRank(player, PM_Rank.Rank.SUPER, sender);
                return true;
            }
            return false;
        }
        if(args.length >= 3)
        {
            ...
            catch(Exception ex)
            {
                ...
                level = PM_Rank.nameToRank(rank).level;
            }
            if(level == 0)
            {
            ...
            if(!PM_Rank.isRankOrHigher(sender, level))
            {
            ...
            PM_Rank.setRank(player, PM_Rank.levelToRank(level), sender);
            return true;
        }
        return false;
    }
