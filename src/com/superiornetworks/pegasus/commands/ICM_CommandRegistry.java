...
public class ICM_CommandRegistry
{
    ...
    public void registerCommands()
    {
        try
        {
            Pattern PATTERN = Pattern.compile("com/superiornetworks/pegasus/commands/(Command_[^\\$]+)\\.class");
            CodeSource codeSource = PegasusMod.class.getProtectionDomain().getCodeSource();
            if (codeSource != null)
            {
                ...
                while ((zipEntry = zip.getNextEntry()) != null)
                {
                    ...
                    if (matcher.find())
                    {
                        try
                        {
                            Class<?> commandClass = Class.forName("com.superiornetworks.pegasus.commands." + matcher.group(1));
