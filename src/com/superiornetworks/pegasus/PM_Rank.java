...
public class PM_Rank
{
    ...
    {
        return (PegasusMod.config.contains("toggles.caching") && PegasusMod.config.getBoolean("toggles.caching"));
    }
        ...
        try
        {
            if(!(player instanceof Player))
            {
                ...
                else
                {
                    ...
                    {
                        if(PM_SqlHandler.getRank(player.getName()).equalsIgnoreCase((rank.name)))
                        {
                            return rank;
                        }
                    }
            ...
            if(PM_Utils.IMPOSTERS.contains((Player) player))
            {
            ...
            try
            {
                for(Rank rank : Rank.values())
                {
                    if(PM_SqlHandler.getRank(((Player) player).getName()).equalsIgnoreCase(rank.name))
                    {
                        if(shouldCache())
                        {
    ...
    public static void unImposter(Player player)
    {
        try
        {
            PM_Utils.IMPOSTERS.remove(player);
            PM_SqlHandler.updateInTable("playerName", player.getName(), false, "imposter", "players");
            PM_SqlHandler.updateInTable("playerName", player.getName(), player.getAddress().getHostString(), "ip", "players");
            if(shouldCache())
            {
                ranks.clear();
                nicks.clear();
    ...
    public static String getTag(Player player)
    {
        ...
        try
        {
            String tag = PM_SqlHandler.getTag(player.getName());
    ...
    public static String getNick(Player player)
    {
        ...
        try
        {
            String nick = PM_SqlHandler.getNick(player.getName());
            if("".equalsIgnoreCase(nick) || nick == null || "off".equalsIgnoreCase(nick))
            {
                if(shouldCache())
                {
    ...
    public static void setRank(Player player, Rank rank, CommandSender sender)
    {
        try
        {
            if(sender == null)
            {
                PM_SqlHandler.updateInTable("playerName", player.getName(), rank.name, "rank", "players");
                if(shouldCache())
                {
                    ranks.put(player.getName(), rank);
            ...
            PM_Utils.adminAction(sender.getName(), message, false);
            PM_SqlHandler.updateInTable("playerName", player.getName(), rank.name, "rank", "players");
            if(shouldCache())
            {
                ranks.put(player.getName(), rank);
        ...
        {
            PegasusMod.plugin.getLogger().severe(ex.getLocalizedMessage());
        }
    }
}
